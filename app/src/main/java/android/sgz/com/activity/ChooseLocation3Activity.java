...
import android.sgz.com.adapter.AddressAdapter;
import android.sgz.com.utils.DataConversionUtils;
import android.sgz.com.utils.DatasKey;
...
public class ChooseLocation3Activity extends AppCompatActivity {
...
//                    doSearchQuery(true, "", location.getCity(), new LatLonPoint(cameraPosition.target.latitude, cameraPosition.target.longitude));
                ...
                try {
                    if (null != loc) {
                        ...
                        if (loc.getErrorCode() == 0) {//可在其中解析amapLocation获取相应内容。
                            location = loc;
                            SPUtil.putString(ChooseLocation3Activity.this, DatasKey.LOCATION_INFO, gson.toJson(location));
        ...
        //recycleview列表监听器
        mAddressAdapter.setOnItemClickLisenter(new IRecycleViewOnItemClickListener() {
            @Override
...
//                        startActivityForResult(new Intent(this, class), SEARCHREQUESTCODE);
                        break;
...
//                        Toast.makeText(MainActivity.this, "定位", Toast.LENGTH_SHORT).show();
                    ...
                    case R.id.bt_send:
                        if (null != mList && 0 < mList.size() && null != mAddressAdapter) {
                            ...
                            intent.putExtra(ReleaseOrderActivity.REQUEST_CHOOSE_LOCATION_ADDRESS_KEY, poiItem.getCityName()+poiItem.getTitle());
                            ...
                            finish();
                            //Toast.makeText(ChooseLocation3Activity.this, "发送：" + poiItem.getTitle() + poiItem.getCityName()+  "  " + poiItem.getSnippet() + "  " + "纬度：" + poiItem.getLatLonPoint().getLatitude() + "  " + "经度：" + poiItem.getLatLonPoint().getLongitude(), Toast.LENGTH_SHORT).show();
                        }
                        break;
                }
            }
